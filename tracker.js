const uParse=require('url').parse;
const net=require('net');
const index=require('./index');
const Buffer=require('buffer').Buffer;
const torrentParser=require('./torrent-parser');
const util=require('./util');
const crypto=require('crypto');

module.exports.peers=async (torrent,callback)=>{
  const url=uParse(torrent.announce.toString('utf8'));

  var client=new net.Socket();
  client.setEncoding('utf8');

  client.on('data',response=>{
    console.log('here2');
    if(torrentParser.respType(response)=='connect'){
      const connResp=parseConnResp(response);
      const aReq=buildAnnoucneReq(connResp.connectionId);
      client.write(
        aReq,
        ()=>{
          console.log('sent connection request');
        }
      );
    }
    else if(torrentParser.respType(response)=='announce'){
      const announceResp=parseAnnounceResp(response);
      callback(announceResp.peers);
    }
  });

  let conn=new Promise((resolve,reject)=>{
    client.connect(
      url.port,
      url.hostname,
      ()=>{
        resolve();
      }
    );
  }).then(()=>{
    console.log('connected');
    console.log(buildConnReq().toString('utf8'));
    client.write(buildConnReq(),()=>{
      console.log('conn req sent');
    });
  }).catch((x)=>{
    console.log(x);
  });
};

module.exports.peers(index,(x)=>{

});




function buildConnReq() { //good

  const buf = Buffer.alloc(16); // 2

  // connection id
  buf.writeUInt32BE(0x417, 0); // 3
  buf.writeUInt32BE(0x27101980, 4);
  // action
  buf.writeUInt32BE(0, 8); // 4
  // transaction id
  crypto.randomBytes(4).copy(buf, 12); // 5

  return buf;
}

function parseConnResp(resp) { //good
  return {
    action: resp.readUInt32BE(0),
    transactionId: resp.readUInt32BE(4),
    connectionId: resp.slice(8)
  }
}

function parseAnnounceResp(resp) { //good
  function group(iterable, groupSize) {
    let groups = [];
    for (let i = 0; i < iterable.length; i += groupSize) {
      groups.push(iterable.slice(i, i + groupSize));
    }
    return groups;
  }

  return {
    action: resp.readUInt32BE(0),
    transactionId: resp.readUInt32BE(4),
    leechers: resp.readUInt32BE(8),
    seeders: resp.readUInt32BE(12),
    peers: group(resp.slice(20), 6).map(address => {
      return {
        ip: address.slice(0, 4).join('.'),
        port: address.readUInt16BE(4)
      }
    })
  }
}

function buildAnnounceReq(connId, torrent, port=6881) {
  console.log('here');
  const buf = Buffer.allocUnsafe(98);

  // connection id
  connId.copy(buf, 0);
  // action
  buf.writeUInt32BE(1, 8);
  // transaction id
  crypto.randomBytes(4).copy(buf, 12);
  // info hash
  torrentParser.infoHash(torrent).copy(buf, 16);
  // peerId
  util.genId().copy(buf, 36);
  // downloaded
  Buffer.alloc(8).copy(buf, 56);
  // left
  torrentParser.size(torrent).copy(buf, 64);
  // uploaded
  Buffer.alloc(8).copy(buf, 72);
  // event
  buf.writeUInt32BE(0, 80);
  // ip address
  buf.writeUInt32BE(0, 80);
  // key
  crypto.randomBytes(4).copy(buf, 88);
  // num want
  buf.writeInt32BE(-1, 92);
  // port
  buf.writeUInt16BE(port, 96);

  return buf;
}
